<template>
  <sys-form :submit-function="submit">
    ...
    <div>
      <sys-label for="stripe-card-input"> Card number </sys-label>
    ...
    <div class="half">
      <sys-label for="stripe-expiration-input"> Expiration </sys-label>
    ...
    <div class="half">
      <sys-label for="stripe-cvc-input"> CVC </sys-label>
...
export default {
  ...
  async fetch() {
    const res = await fetch(
      `${process.env.API_URL}/geography/countries?page[size]=1000`,
      {
        method: "GET",
        headers: new Headers({
